    ...
    while(!m_jobCancel)
    {
		...
		if (idx == buffer[0]) //Frame length
		{
            dumpHex("Received frame", buffer, idx);
             ...
             // TODO delete below. They are for CRC debug
             /**
        if (idx == 32 && buffer[30]==0xFC && buffer[31]==0x3F){
            printf("This is RS response\n======================================================\n");
            byte testData[32] = {0x20,0x06,0x01,0x01, 0x0,0x0,0x0,0x0, 0x08,0x1E,0xBE,0x28, 0x80,0x05,0x98,0x00,
            0x00,0x01,0x00,0x01, 0x00,0x01,0x00,0x09, 0x10,0x21,0x01,0x00, 0x64,0x00,0xFD,0x0A};
            std::vector<byte> myframeToSend;
            for(int i=0;i<32;i++){
                buffer[i] =testData[i];
            }
        }
