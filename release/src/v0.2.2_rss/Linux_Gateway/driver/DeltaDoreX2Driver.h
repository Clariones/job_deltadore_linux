...
class DeltaDoreX2Driver: public AcknowledgmentListener, public EndTransactionListener, public NodeDiscoveredListener
{
    ...
    public:
        ...
        cJSON* setNodeName(int network, int node, const char* region, const char* name);
        cJSON* setNodeDeviceType(int network, int node, const char* deviceType);

        cJSON* debugPrintRead(bool enablePrint);

    public:
        void init(const char* devName);
        void waitAck(int seconds);
        ...
        int queryRoolershutterPhyState(int network, int node, DevicePhysicalState& result);
        int queryLightPhyState(int network, int node, DevicePhysicalState& result);
        bool getNodeRealType(int network, int node, DevicePhysicalState& result);
    protected:
        // callbacks
        ...
        void setContextRequestNetwork(int network) { printf("setContextRequestNetwork(%d)\n",network);  contextRequestNetwork = network;}

        int getContextRequestNode() { return contextRequestNode;}
        void setContextRequestNode(int node) { printf("setContextRequestNode(%d)\n",node); contextRequestNode = node;}
    ...
    private:
        ...
        DeltaDoreDeviceInfo idleDeviceInfo;
        FILE * fdStorage;
