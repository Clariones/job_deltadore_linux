本次修改内容：
1. 改变了状态上报的结果。
	如果查询失败，不认为真的失败，而是信号不好，所以不更改设备类型。仅报信号=0. 这样不阻止命令下发。
2. 增加了 设置设备类型 的命令
	因为设备查询不可靠，所以增加了手工指定设备类型的命令。

部署说明：

1. 概要说明
	目录 so 下是达泰多驱动的SO文件
	目录 CmdClient 是 webUI 使用的TCP工具
...
3. 部署WEB UI相关文件
	1. 将 CmdClient 目录下的 CmdClient 拷贝到 /root 目录下
...
4. WEB UI相关文件详细说明
	...
	4. www/fdti_driver.sh
		这个文件是FDTI控制命令服务端脚本, 会调用/root/CmdClient 来发送内部命令。 是新增文件，拷贝到 /www即可
	...
	7. Cmdclient/CmdClient
		这个文件是用来发送内部消息的。
		它在 www/fdti_driver.sh 被调用：
			/root/CmdClient 127.0.0.1 1800 "$d"
		三个参数依次是：
			IP 地址： 默认是本机地址 127.0.0.1. 也支持其他地址。
			端口号：默认是 1800， 这个参数需要和 /etc/luoping/save/dtdconfig.json 中参数 webCommandPort 一致
			命令字符串：这个是从web UI 发送过来的命令。 手工使用时，可以使用 help 获得相关的命令说明。 例如：/root/CmdClient 127.0.0.1 1800 "help"
...
5. 设备驱动相关文件详细说明
	...
	2. config/save/dtdconfig.json
		达泰多设备驱动的配置文件。 内容是：
			...
			"webCommandPort": 接收WebUI 命令的TCP端口值，默认是1800, 需要和www/fdti_driver.sh中使用的端口号保持一致。
